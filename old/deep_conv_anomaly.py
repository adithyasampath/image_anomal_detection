...
class AnomalyPredictor:
    ...
    def IsImageHasAnomaly(self, filePath, filename):
        ...
        self.save_image(os.path.join(self.test_save_to,"predicted"),"predicted_" + filename,predicted_image[0].astype(np.float32)* 255.0)
        self.save_image(os.path.join(self.test_save_to,"original"),"original_" + filename,validation_image[0].astype(np.float32)* 255.0)
    ...
    def create_model(self):
        ...
        conv10 = Conv2D(3, (3, 3), activation='sigmoid', padding='same',kernel_regularizer=regularizers.l2(0.01))(conv9)
    ...
    def train_model(self):
        ...
        # early_stopping = keras.callbacks.EarlyStopping(monitor='loss', min_delta=0.0001, patience=2, verbose=1, mode='auto')
        ...
        callbacks.append(keras.callbacks.TensorBoard(log_dir='./logs',
                ...
                write_graph=True,
                write_images=True))
        self.autoencoder.compile(metrics=['mae', 'acc'],optimizer='adam', loss='binary_crossentropy')
    ...
    def set_threshold(self):
        ...
        for valid_image in self.train_generator:
            ...
            (mse_value, diff) = compare_ssim(grayA, grayB, full=True)   #self.mse(predicted_image[0], valid_image[0])
        ...
        self.threshold = np.percentile(np.asarray(all_mses),60)
        median = {}
    ...
    def load_weights(self):
        self.autoencoder.load_weights(self.weights_dir)
        self.autoencoder.compile(metrics=['mae', 'acc'],
